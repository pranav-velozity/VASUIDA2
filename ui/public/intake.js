...
(function intakeModule() {
  ...
  // ---- Private api() helper — includes Clerk auth token ----
  ...
  function renderIntake() {
    ...
    tb.querySelectorAll('input[data-f]').forEach(inp => {
      ...
      inp.addEventListener('change', async e => {
        ...
        try {
          const res = await authFetch(`${apiBase}/records`, {
  ...
  if (__btnDelSel) __btnDelSel.onclick = async () => {
    ...
    if (apiBase) {
      ...
      if (pairs.length) {
        try {
          await authFetch(`${apiBase}/records/delete`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
        ...
        if (apiBase && payload.length) {
          try {
            ...
            for (let i = 0; i < payload.length; i += CHUNK) {
              ...
              totalInserted += j.inserted ?? chunk.length;
  ...
  (function wireUploadBins() {
    ...
    file.addEventListener('change', async (e) => {
      ...
      try {
        ...
        const resp = await authFetch(`${apiBase}/bins/weeks/${ws}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteUIDs() {
    ...
    try {
      const res = await authFetch(`${apiBase}/records/delete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
