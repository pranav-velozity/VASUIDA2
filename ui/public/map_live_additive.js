...
(function () {
  ...
  function buildMapData(lanes, containers, plan, receiving, appliedByPO){
    ...
    // Received POs — from receiving table rows (each has po_number)
    const receivedPOs=new Set((receiving||[]).map(r=>String(r.po_number||'').trim()).filter(Boolean));

    // Applied units by zendesk
    ...
    // Vessel groups (transit only — have departed date)
    const vesselGroups=new Map();

    // Location groups (static)
    ...
    for(const lane of lanes){
      ...
      // Static location — not in transit
      if(stage!=='transit'){
