...
(function () {
  ...
  async function getClerkToken() {
    if (window.Clerk && window.Clerk.session) {
      try { return await window.Clerk.session.getToken(); } catch(e) {}
    }
    return null;
  }
...
function nowDateTimeLocalValue() { return dtLocalNow(); }
  ...
  // If a state-based cartonsOut helper returns real data, use it (avoids an extra API call).
  // IMPORTANT: only replace M.cartonsOutByPO if the derived map is non-empty — otherwise
  // we'd overwrite the correctly API-fetched map with an empty one, wiping Carton Out values.
  try {
    const ws = getWeekStart();
    if (!ws) return;
    if (ws !== M.ws) return;
    if (typeof computeCartonsOutByPOFromState === 'function') {
      const derived = computeCartonsOutByPOFromState(window.state || {});
      if (derived && derived.size > 0) {
        M.cartonsOutByPO = derived;
        try { renderCurrentSupplierView(); } catch {}
      }
      // If derived is empty, keep existing M.cartonsOutByPO from the API fetch — don't overwrite.
    }
  } catch {}
});
...
// Expose global so show('#receiving') can call directly without hash manipulation
  ...
  // Wait for state:ready (fired after Clerk auth + setWeek completes)
  // This ensures weekStart is populated before we try to load data
  if (window.state && window.state.weekStart) {
    tick(); // state already ready (e.g. page reload on #receiving)
  } else {
    window.addEventListener('state:ready', function onReady() {
      window.removeEventListener('state:ready', onReady);
      tick();
    });
  }
})();
